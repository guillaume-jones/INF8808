...
var _yearButtonJs = require("./scripts/yearButton.js");
...
(async function(d3) {
    ...
    const year1 = (0, _yearButtonJs.drawDropdown)(years, mapsize.width);
    (0, _clickHandlers.dropDownClickHandler)(redrawVizForYear);
    // Call draw graphs
    redrawVizForYear(year1);
})(d3);

},{"./scripts/preprocess":"ko2Fr","./scripts/mapViz":"cyjxE","./scripts/yearButton.js":"hDjna","./scripts/clickHandlers":"blWZ6","./scripts/geography":"iRz4J","./scripts/lineChart":"kHSI7","./scripts/areaChart":"apy0w","./scripts/barChartViz.js":"lsnFW"}],"ko2Fr":[function(require,module,exports) {
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"hDjna":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
...
function addLabels(g, width, height, name, neighborhood) {
    ...
    // Title
    if (name) g.append("text").text(name + " - " + neighborhood).attr("x", width / 4).attr("y", 10);
    else g.append("text").text("Moyenne de tous les compteurs").attr("x", width / 4).attr("y", 10);
}
...
function drawLineChart(width, height, averageData, counterData) {
    ...
    if (counterData) group.append("path").datum(counterData.counts).attr("fill", "rgba(0, 0, 0, 0)").attr("stroke", "rgba(18, 81, 153, 1)").attr("stroke-width", 1.5).attr("d", d3.line().x(function(d) {
        return xScale(d.index);
    }).y(function(d) {
        return yScale(d.value);
    }));
...
function addLabels(g, width, height, name, neighborhood) {
    ...
    g.append("text").text("Comptes").attr("x", 10).attr("y", height / 2).attr("transform", "rotate(-90)");
    // Title
    if (name) g.append("text").text(name + " - " + neighborhood).attr("x", width / 3).attr("y", 10);
    else g.append("text").text("Moyenne de tous les compteurs").attr("x", width / 3).attr("y", 10);
}
function drawAreaChart(width, height, averageData, counterData) {
    ...
    if (counterData) group.append("path").datum(counterData.counts).attr("fill", "rgba(77, 149, 232, 0.5)").attr("stroke", "rgba(18, 81, 153, 0.5)").attr("stroke-width", 1.5).attr("d", d3.area().x(function(d) {
        return xScale(d.index);
    }).y0(height).y1(function(d) {
        return yScale(d.value);
    }));

...
function generateData() {
  var size = d3.randomUniform(1, 11)();
  var uniformGenerator = d3.randomUniform(1, 100);
  var data = Array.from({
    length: size
  }, function () {
    return {
...
function updateInfoPanel() {
  var nbOfDots = getDotCount();
  d3.select('.dot-count').text(nbOfDots);
  d3.select('.dot-label').text(nbOfDots > 1 ? 'points' : 'point');
}
/**
